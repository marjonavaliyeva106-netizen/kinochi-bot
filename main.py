...
# Kanallar ro'yxati (Masalan: .env ichida CHANNELS=@kanal1,@kanal2 ko'rinishida yozing)
channels_env = os.getenv("CHANNELS", "@nexora_startup,@kinoch_ibot1")
CHANNELS = [id.strip() for id in channels_env.split(",") if id.strip()]

# Kanal URLlari (Masalan: .env ichida CHANNEL_URLS=https://t.me/kanal1,https://t.me/kanal2)
...
# --- MA'LUMOTLAR BAZASI ---
# Local development uchun DB yo'lini moslash
...
def admin_kb():
    kb = [
        ...
        [InlineKeyboardButton(text="🎬 Barcha kinolar", callback_data="admin_list_0")], # 0-sahifadan boshlash
...
# --- KINO QIDIRISH (FOYDALANUVCHI - OXIRIDA BO'LISHI KERAK) ---
@dp.message()
async def search_movie(message: Message):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if results:
            ...
            await message.answer("❌ Hech narsa topilmadi. Kodni kodi (raqamini) to'g'ri kiriting.")
